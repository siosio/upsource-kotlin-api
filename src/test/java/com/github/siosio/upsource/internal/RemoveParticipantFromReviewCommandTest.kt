...
internal class RemoveParticipantFromReviewCommandTest : UpsourceApiTestSupport() {

  @Test
  fun removeParticipantFromReview() {
    ...
    // -------------------------------------------------- execute
    sut.send(RemoveParticipantFromReviewCommand(
        ParticipantInReviewRequest(ReviewId("prj", "review-1"), ParticipantInReview("user1", RoleInReviewEnum.Reviewer))))

    // -------------------------------------------------- assert
    assertThat(httpPost.value.uri.toASCIIString(), `is`("http://testserver/~rpc/removeParticipantFromReview"))
